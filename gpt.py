...
class GPT():
    def __init__(self, temperature=0.9, is_llama=True, device=0):
    ...
    def generate(self, prompt, max_tokens, model, stop_tokens=None, system_prompt="", t=None):
        if not t:
            t = self.temperature
        # try:
    ...
    def get_probs(self, prompt, option_dict, model, max_tokens=8, n=1, max_iters=5, system_prompt=""):
        prompt = self.trim_prompt(prompt)
    ...
    def trim_prompt(self, prompt):
        ...
        delete_turn_num = 0
        while len(self.tokenize(prompt)) > (4096 - 50 - 5):
